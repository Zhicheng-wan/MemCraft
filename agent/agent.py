...
MINING:
  {"name": "find_and_mine_block", "params": {"block_name": "<n>", "count": <1-5>}}
    Examples: dirt, oak_log, acacia_log, birch_log, cobblestone

CRAFTING:
  {"name": "craft_item", "params": {"item_name": "<n>", "count": <int>}}
  Auto-places crafting table if needed (requires 4 extra planks for the table).

  WOODEN PICKAXE RECIPE (follow exactly):
    1. Mine 4 logs: find_and_mine_block with any log type (oak_log, acacia_log, etc.)
    2. Craft ALL logs into planks: craft_item <type>_planks count=4 (gives 16 planks)
       IMPORTANT: plank type must match log type! acacia_log -> acacia_planks, oak_log -> oak_planks
    3. Craft sticks: craft_item stick count=1 (uses 2 planks, gives 4 sticks)
    4. Craft pickaxe: craft_item wooden_pickaxe count=1 (needs 3 planks + 2 sticks + crafting table)
       The crafting table is auto-crafted and placed if you have 4+ planks available.
       So you need: 3 planks (pickaxe) + 4 planks (table) + 2 planks (sticks) = 9+ planks before step 3-4.
...
- Use find_and_mine_block to mine specific blocks. Do NOT use dig_block_below.
- If an action fails, try a DIFFERENT action. NEVER repeat a failed action.
...
class BaseAgent:
    ...
    def wait_for_bot(self, timeout: int = 30):
    ...
    def check_goal_complete(self, goal: str, raw_obs: dict) -> bool:
        ...
        craft_match = re.search(r'craft\s+(?:a\s+|an\s+)?(\w+[\w_]*)', goal_lower)
        if craft_match:
            target = craft_match.group(1)
            for item_name, item_count in session_inv.items():
                if target in item_name and item_count >= 1:
                    return True
